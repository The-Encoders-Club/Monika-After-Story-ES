...
init -100 python in mas_selspr:
    ...
    PROMPT_MAP = {
        "choker": {
            ...
            "change": "¿Podrías cambiar tu gargantilla?",
            "wear": "¿Podrías usar una gargantilla?",
        },
        "clothes": {
            "_not_group": True,
            "_ev": "monika_clothes_select",
            "change": "¿Podrías cambiarte de ropa?",
            
        },
        "earrings": {
            "_ev": "monika_earrings_select",
            "_min-items": 1,
            "change": "¿Podrías cambiar tus aretes?",
            "wear": "¿Podrías usar aretes?",
        },
        "hair": {
            "_not_group": True,
            "_ev": "monika_hair_select",
            "change": "¿Podrías cambiar tu estilo de cabello?",
            
        },
        "hat": {
            "_ev": "monika_hat_select",
            "_min-items": 1,
            "change": "¿Podrías cambiar tu sombrero?",
            "wear": "¿Podrías usar un sombrero?",
        },
        "left-hair-clip": {
            "_ev": "monika_hairclip_select",
            "_min-items": 1,
            "change": "¿Podrías cambiar tu horquilla?",
            "wear": "¿Podrías usar una horquilla?",
        },
        "left-hair-flower": {
            "_ev": "monika_hairflower_select",
            "_min-items": 1,
            "change": "¿Podrías cambiar la flor de tu cabello?",
            "wear": "¿Podrías usar una flor en tu cabello?",
        },
        "necklace": {
            "_ev": "monika_necklace_select",
            "_min-items": 1,
            "change": "¿Podrías cambiar tu collar?",
            "wear": "¿Podrías usar un collar?",
        },
        "ribbon": {
            "_ev": "monika_ribbon_select",
            "_min-items": 1,
            "_rule": _rule_ribbon,
            "change": "¿Podrías atarte el cabello con otra cosa?",
            "wear": "¿Podrías atarte el cabello?",
        },
    }
    ...
    class MASSelectableSprite(object):
        ...
        def _check_dlg(self, dlg):
            if dlg is not None and not renpy.has_label(dlg):
                raise Exception("label '{0}' no existe".format(dlg))
    ...
    class MASSelectableAccessory(MASSelectableSprite):
        ...
        def __init__(self,
            ...
            if not isinstance(_sprite_object, MASAccessoryBase):
                raise Exception("no es una acs: {0}".format(group))
            if remover and store.mas_selspr._has_remover(group):
                raise Exception(
                    "no puede haber más de 1 remover por grupo: '{0}'".format(
                        group
                    )
                )
    ...
    class MASSelectableHair(MASSelectableSprite):
        ...
        def __init__(self,
            ...
            if type(_sprite_object) != MASHair:
                raise Exception("no es un estilo de cabello: {0}".format(group))
    ...
    class MASSelectableClothes(MASSelectableSprite):
        ...
        def __init__(self,
            ...
            if type(_sprite_object) != MASClothes:
                raise Exception("no es una ropa: {0}".format(group))
...
init -10 python in mas_selspr:
    ...
    generic_sel_dlg_quips = [
        ...
        "¡Yo estaba pensando lo mismo, [player]!",
    ...
    disable_sel_dlg_quips = {
        DISB_NONE: None,
        DISB_HAIR_BC_CLOTH: [
            "Ese estilo de cabello no combina con mi ropa.",
            "No creo que este estilo de cabello funcione realmente con este conjunto.",
            "Ese estilo de cabello no combina bien con este atuendo.",
            "Creo que este estilo de cabello funciona mejor con un conjunto diferente."
        ],
        DISB_ACS_BC_HAIR: [
            "Eso realmente no funciona con mi cabello.",
            "No creo que esto funcione con mi cabello.",
            "Esto podría funcionar con un estilo de cabello diferente.",
    ...
    def init_selectable_acs(
        ...
        if acs.name in ACS_SEL_MAP:
            raise Exception("ACS ya se puede seleccionar: {0}".format(acs.name))
    ...
    def init_selectable_clothes(
        ...
        if clothes.name in CLOTH_SEL_MAP:
            raise Exception(
                "La ropa ya se puede seleccionar: {0}".format(clothes.name)
            )
    ...
    def init_selectable_hair(
        ...
        if hair.name in HAIR_SEL_MAP:
            raise Exception("El cabello ya se puede seleccionar: {0}".format(hair.name))
            ...
            for item in select_map.itervalues():
                if use_old or item.selected:
                    ...
                    try:
                        ...
                        store.mas_utils.mas_log.warning("MAL CABELLO: " + repr(e))
            ...
            for item in select_map.itervalues():
                if use_old or item.selected:
                    ...
                    try:
                        ...
                        store.mas_utils.mas_log.warning("ROPA MALA: " + repr(e))
...
style filter_dropdown_down is generic_button_light:
    ...
    child Fixed(
        HBox(
            Null(width=8),
            Text("Filtro", color=mas_ui.light_button_text_idle_color, outlines=[]),
            Null(width=12),
    ...
    hover_child Fixed(
        HBox(
            Null(width=8),
            Text("Filtro", color=mas_ui.light_button_text_hover_color, outlines=[]),
            Null(width=12),
...
style filter_dropdown_down_dark is generic_button_dark:
    ...
    child Fixed(
        HBox(
            Null(width=8),
            Text("Filtro", color=mas_ui.dark_button_text_idle_color, outlines=[]),
            Null(width=12),
    ...
    hover_child Fixed(
        HBox(
            Null(width=8),
            Text("Filtro", color=mas_ui.dark_button_text_hover_color, outlines=[]),
            Null(width=12),
...
label mas_selector_sidebar_select(items, select_type, preview_selections=True, only_unlocked=True, save_on_confirm=True, mailbox=None, select_map={}, add_remover=False, remover_name=None, filter_map=None):
    ...
    python:
        ...
        if not store.mas_selspr.valid_select_type(select_type):
            raise Exception(
                "constante de selección no válida: {0}".format(select_type)
            )
...
label mas_selector_generic_sidebar_select_acs(acs_type, use_acs=None, set_compat_flags=True, launch_exp="monika 1eua", idle_exp=None, sel_group=None, idle_dlg=None):
    python:
        ...
        if sel_group is None:
            sel_group = acs_type
        if idle_dlg is None:
            idle_dlg = "¿Qué {0} te gustaría que usara?".format(acs_type)
...
label monika_hair_select:
    ...
    python:
        ...
        mailbox = store.mas_selspr.MASSelectableSpriteMailbox(
            "¿Qué estilo de cabello te gustaría que use?"
        )
        sel_map = {}
...
label monika_ribbon_select:
    python:
        ...
        use_acs.append(store.mas_selspr.create_selectable_remover(
            "cinta",
            "cinta",
            "Cinta de cabello básica"
        ))

        mailbox = store.mas_selspr.MASSelectableSpriteMailbox(
            "¿Qué cinta para el cabello te gustaría que usara?"
        )
        sel_map = {}
...
label monika_hairclip_select:
    call mas_selector_generic_sidebar_select_acs ("left-hair-clip", idle_dlg="¿Qué horquilla te gustaría que llevara?")
    return
...
label monika_hairflower_select:
    python:
        ...
        use_acs.append(store.mas_selspr.create_selectable_remover(
            "flor de cabello izquierda",
            "flor de cabello izquierda"
        ))

        mailbox = store.mas_selspr.MASSelectableSpriteMailbox(
            "¿Qué flor quieres que me ponga en el cabello?"
        )
        sel_map = {}
