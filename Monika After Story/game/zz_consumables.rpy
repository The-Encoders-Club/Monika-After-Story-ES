...
init python:
    ...
    def mas_generateShoppingList(low_cons_list=None):
        ...
        START_TEXT = (
            "Hola, [player],\n"
            "Solo te hago saber que me estoy quedando sin un par de cosas.\n"
            "No te importaría conseguir más para mí, ¿verdad?\n\n"
            "Aquí hay una lista de lo que se me está acabando:\n"
        )
...
init 1 python:
    ...
    mas_consumable_christmascookies = MASConsumable(
        ...
        consumable_type=store.mas_consumables.TYPE_FOOD,
        disp_name="galletas de navidad",
        dlg_props={
            mas_consumables.PROP_OBJ_REF: "platos",
            mas_consumables.PROP_PLUR: True
        },
...
label mas_consumables_generic_get(consumable):
    ...
    python:
        ...
        if container:
            line_starter = renpy.substitute("Voy por [consumable.disp_name].")
            ...
            a_an = "poco" if plur else mas_a_an(consumable.disp_name, ignore_case=True)
    ...
    if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
        m 1hua "¡Volví! {w=1.5}{nw}"
    else:

        m 1eua "Bien, ¿qué más deberíamos hacer hoy?"
    return
...
label mas_consumables_generic_finish_having(consumable):
    ...
    python:
        ...
        dlg_map = {
            mas_consumables.PROP_CONTAINER: {
                0: "Voy a guardar este [container].",
                1: "Voy por otro [container]."
            },
            mas_consumables.PROP_OBJ_REF: {
                0: "Voy a poner esto lejos.",
                1: "Voy por otro [obj_ref]."
            },
            "else": {
                0: "Voy a poner esto lejos.",
                1: "Voy a tomar otro."
    ...
    if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
        ...
        m 1eua "Okey, ¿qué más deberíamos hacer hoy?"
    return
...
label mas_consumables_generic_finished_prepping(consumable):
    ...
    if (not mas_canCheckActiveWindow() or mas_isFocused()) and not store.mas_globals.in_idle_mode:
        $ is_are = "están" if plur else "está"
        m 1esd "Oh, mi [consumable.disp_name][plur] [is_are] listo."
        m 1eua "Espera un momento."
    else:


        m 1eua "Voy por mi[plur] [consumable.disp_name][plur]. Vuelvo enseguida.{w=1}{nw}"
...
label mas_consumables_refill_explain:
    ...
    m 1eksdla "Supongo que me debo estar perdiendo algo, porque no puedo replicar el sabor."
...
label mas_consumables_generic_running_out(consumable):
    ...
    if amt_left > 0:
        python:
            ...
            if container:
                line_ender = renpy.substitute("[container][plur] de [consumable.disp_name][plur].")


            elif obj_ref:
                line_ender = renpy.substitute("[obj_ref][plur] de [consumable.disp_name][plur].")


            else:
                line_ender = renpy.substitute("[consumable.disp_name][plur].")
        ...
        m 3eud "Solo quería hacerte saber que tengo [about][amt_left] [line_ender]"

        if not renpy.seen_label("mas_consumables_refill_explain"):
            call mas_consumables_refill_explain
    else:

        m 3eud "Solo quería informarte que me quedé sin [consumable.disp_name][plur]."

    m 1eka "No te importaría conseguirme más, ¿verdad?"
    return
...
label mas_consumables_generic_critical_low(consumable):
    python:
        ...
        if container:
            line_ender = renpy.substitute("[container] de [consumable.disp_name].")


        elif obj_ref:
            line_ender = renpy.substitute("[obj_ref] de [consumable.disp_name].")


        else:
            line_ender = renpy.substitute("poco de [consumable.disp_name].")

    m 1euc "Hey, [player]..."
    m 3eua "Solo tengo un [line_ender]"

    if not renpy.seen_label("mas_consumables_refill_explain"):
        call mas_consumables_refill_explain

    m 3eka "¿Te importaría traerme un poco más alguna vez?"
    m 1hua "Gracias~"
    return
...
label mas_consumables_generic_queued_running_out_dlg(low_cons):
    ...
    if len(low_cons) > 2:
        ...
        m 3eua "Así que espero que no te importe, pero te dejé una lista de cosas en la carpeta de 'characters'."
        $ them = "lo"
    else:
...
label mas_consumables_remove_thermos:
    ...
    if store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
        m 1eua "Voy a guardar este termo. Vuelvo enseguida.{w=1}{nw}"
    else:
...
label mas_consumables_candycane_finish_having:
    ...
    if not get_more:
        ...
        if not store.mas_globals.in_idle_mode and (not mas_canCheckActiveWindow() or mas_isFocused()):
            ...
            m 3eua "Dame un momento."

        elif store.mas_globals.in_idle_mode or (mas_canCheckActiveWindow() and not mas_isFocused()):
            m 1esd "Oh, me he comido mis bastones de caramelo. {w=1}{nw}"
            m 1eua "Voy a buscar más. Volveré enseguida. {w=1}{nw}"
...
init python:
    addEvent(
        Event(
            ...
            category=['reservas'],
            prompt="¿Te estás quedando sin algo?",
...
label monika_consumables_check:
    ...
    if len(low_cons) < 2 and random.random() > 0.5:
        if not low_cons:
            m 3eua "Oh, {w=0.1}no me hace falta nada de momento, [player]... {w=0.3}{nw}"
            extend 3hua "pero me aseguraré de avisarte si me hace falta algo~"
        else:

            $ items_running_out_of = low_cons[0].disp_name
            m 3rusdlb "Oh, {w=0.1}¡me alegro de que preguntes!"
            m 1rksdla "Me estoy quedando sin [items_running_out_of]."
            m 1eka "Apreciaría si pudieras conseguirme más~"

        return

    m 1rtd "Emm... {w=0.3}{nw}"
    extend 3eua "déjame mirar.{w=0.2}.{w=0.2}.{w=0.2}{nw}"
    ...
    m 1hub "¡Estoy de vuelta!"

    if len(low_cons) > 2:
        $ mas_generateShoppingList(low_cons)
        m 3rksdla "Se me están acabando unas cuantas cosas..."
        m 3eua "Espero que no te importe, pero te he dejado una lista con esas cosas en la carpeta 'characters'."
        m 1eka "No te importa ir a por unos cuantos de cada para mí, ¿verdad?"
        ...
        m 3rksdla "Me estoy quedando sin [items_running_out_of]."
        m 1eka "No te importa ir a por unos cuantos para mí, ¿verdad?"
    else:

        m 3eua "No me falta nada de momento, [player]... {w=0.3}{nw}"
        extend 3hua "pero me aseguraré de avisarte si me hace falta algo~"
